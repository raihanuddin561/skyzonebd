...
export async function autoGenerateProfitReport(orderId: string): Promise<{
  ...
  try {
    ...
    // doubles that order's recognized revenue/COGS. The real guard is the
    // `@@unique([orderId])` constraint on ProfitReport (prisma/schema.prisma)
    // — a concurrent second create() throws Prisma's unique-violation error
    // (P2002), which the catch below treats as "already generated by
    // someone else" instead of a real failure. NOTE: this constraint is
    // prepared in the schema but requires a migration to take effect in the
    // database — until that migration runs, this catch is a no-op safety
    // net and the plain read-check above remains the only protection.
    let report;
    try {
